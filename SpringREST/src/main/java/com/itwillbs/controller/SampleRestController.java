...
// @RestController : 해당 클래스를 REST 방식 데이터 처리를 수행하는 클래스
@RestController
...
public class SampleRestController {
	...
	public Integer doB() {
		...
		// 문자 데이터를 리턴
		// String을 제외한 기본형 타입 리턴은 참조형 타입으로 변경 (권장)
		return 1000;
	}
	...
	public List<MemberVO> doD() {
		...
		// 객체를 JSON 타입으로 변환 (자바 직렬화 : 자바 시스템 내부의 객체를 외부에서 사용되는 데이터 형태로 변환)
	...
	public String doInfo(@RequestBody MemberVO vo) {
		// @RequestBody : 브라우저에 전달되는 JSON 데이터를 특정 객체로 변환
		logger.info(" doInfo() 호출 ");
		logger.info(" vo : " + vo);
	...
	public ResponseEntity<Void> doG() {
		...
		// ResponseEntity : 데이터, HTTP 상태 코드를 직접 제어하는 클래스
